...
open class PostViewModel(private val user: String, private val postId: String) : ViewModel() {
    ...
    open var likes = MutableLiveData<Int>()
    open var numComments = MutableLiveData<Int>()
    open var comments = MutableLiveData<List<String>>()
    ...
    fun setupListener(){
        ...
        //future= loadUserData()
        future = dbAPI.searchUserInDatabase(user) // TODO searchUser(user, postId)
        future.thenAccept{
            username.value= it.username
            //likes.value = it.profile.posts
            numComments.value = it.profile.nbFollowers
            //comments.value = it.profile.nbFollowings
